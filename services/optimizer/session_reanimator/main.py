...
async def resume_session(
    ...
    Auth strategy:
      ...
      the public /api/sessions/:id/resume route) was an identity-spoof
      surface if MCP_AUTH_DEV_MODE leaked into a real cluster. It's
      gone. Local dev that needs reanimator behaviour must set
      MCP_AUTH_SIGNING_KEY (a `openssl rand -hex 32` value works); the
      assert_production_safe() check at startup also requires this in
      production-mode bootstraps.
    """
