...
def write_seed_sql(out_path: Path, fixtures_relpath: str) -> None:
    ...
    """
    lines: list[str] = []
    lines.append("-- Mock ELN — seed data loader.")
    lines.append("-- Auto-generated by services/mock_eln/seed/generator.py.")
    lines.append("-- DO NOT EDIT BY HAND. Re-run the generator instead.")
    lines.append("--")
    lines.append("-- Gating: this loader only runs when the postgres GUC")
    lines.append("--   app.mock_eln_enabled = 'on'")
    lines.append("-- is set on the session or the database. Set it via:")
    lines.append("--   psql -v ON_ERROR_STOP=1 \\\\")
    lines.append("--     -c \"SET app.mock_eln_enabled = 'on';\" \\\\")
    lines.append("--     -f db/seed/20_mock_eln_data.sql")
    lines.append("-- or set it persistently with:")
    lines.append("--   ALTER DATABASE chemclaw SET app.mock_eln_enabled = 'on';")
    lines.append("--")
    lines.append("-- Re-running is safe — the gated block TRUNCATEs the mock_eln")
    lines.append("-- tables (CASCADE) before \\copying so the resulting state is")
    lines.append("-- byte-identical to a fresh run with the same fixtures.")
    lines.append("")
    lines.append(r"\set ON_ERROR_STOP on")
    lines.append("")
    lines.append("-- Use a top-level psql variable so the gating block resolves")
    lines.append("-- before \\copy expansion.")
    lines.append("SELECT")
    lines.append(
        "  CASE WHEN coalesce(current_setting('app.mock_eln_enabled', true), 'off') = 'on'"
    )
    lines.append("       THEN 'on' ELSE 'off' END AS mock_eln_state \\gset")
    lines.append("")
    lines.append(r"\if :{?mock_eln_state}")
    lines.append(r"\if :{?mock_eln_state}")
    # Use the simpler approach: a DO block to short-circuit when off.
    lines.append("")
    # Replace with the cleaner RAISE NOTICE + \q-on-disabled approach:
    # The cleanest portable gate that works for both \copy (psql) and DO blocks
    # is a psql conditional driven by :mock_eln_state.
    lines: list[str] = [
