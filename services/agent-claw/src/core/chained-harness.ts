// Multi-turn agent orchestration shared by:
//   - POST /api/sessions/:id/plan/run        (chained execution of an active plan)
//   - POST /api/sessions/:id/resume          (header-trust auto-resume)
//   - POST /api/internal/sessions/:id/resume (JWT-trust auto-resume)
//
// Auto-chains harness turns until completion / max_steps cap / session-budget
// trip / awaiting_user_input. Per-iteration Paperclip reservation mirrors
// the chat route so daily-USD caps apply uniformly to long autonomy runs.
//
// Exported as `runChainedHarness`. The matching tests live in
// tests/integration/chained-execution.test.ts; they import this directly.
...
export interface ChainedHarnessOptions {
   ...
   *  routes/chat. When omitted (PAPERCLIP_URL unset in dev), the in-process
...
/**
 * Run the harness one or more times against a session, auto-chaining
 * until completion / max_steps cap / session-budget trip. Wraps the
 * inner loop in a request context so every outbound MCP call inside the
 * iteration tags itself with the correct user identity.
 */
...
async function _runChainedHarnessInner(
  ...
  while (autoTurns < cap) {
    ...
    // Per-iteration Paperclip reservation. Mirrors routes/chat so the daily
    ...
    try {
      ...
      // routes/chat so a mid-chain ask_user with a SMILES / NCE-ID /
      ...
      // USD_PER_TOKEN_ESTIMATE constant (routes/chat uses the same).
