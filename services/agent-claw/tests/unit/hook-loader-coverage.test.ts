...
// Gap-plan H0.9 added scheduled-substance-gate on pre_tool (22 total;
...
describe("hook loader coverage", () => {
  ...
  it("registers all 22 known hook implementations at the right points", async () => {
    ...
    expect(lc.count("pre_tool")).toBe(3); // budget-guard, foundation-citation-guard, scheduled-substance-gate
    ...
    // Sanity sum: 2 + 3 + 3 + 1 + 1 + 1 + 1 + 9 + 1 = 22 hooks total.
