...
def test_conformer_ensemble_happy_path(client):
    """/conformer_ensemble routes through the workflow engine: each conformer
    is xtb-optimised after CREST, so the returned energies are post-opt and
    the Boltzmann weights reflect those refined energies.
    """
    ...
    # Energies are POST-opt, so they match the optimisation stdout
