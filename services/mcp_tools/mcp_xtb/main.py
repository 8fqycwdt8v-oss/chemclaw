...
_XTB_TIMEOUT = 120  # seconds; heavier endpoints override
_MAX_CONFORMERS = 100
_MAX_MD_STEPS = 10_000
_MAX_SCAN_POINTS = 200
...
def _solvent_flags(model: str | None, name: str | None) -> list[str]:
...
def _run_xtb(args: list[str], cwd: Path, timeout: int = _XTB_TIMEOUT) -> subprocess.CompletedProcess[str]:
...
def _parse_homo_lumo(stdout: str) -> tuple[float | None, float | None]:
    homo = lumo = None
    for line in stdout.splitlines():
        if "HOMO-LUMO GAP" in line:
            # Example: "          | HOMO-LUMO GAP               4.123 eV"
            parts = line.split()
...
def _check_cache(
    *,
    ...
    """Return a cached lookup or None when caller should compute fresh."""
    if force_recompute:
        return None
    return qm_lookup(
        method=method,
...
def _parse_thermo(stdout: str) -> dict[str, float]:
    out: dict[str, float] = {}
    for line in stdout.splitlines():
        s = line.strip()
        if "zero point energy" in s.lower():
            out["zpe_hartree"] = _last_float(s)
        elif "total enthalpy" in s.lower():
            out["h298"] = _last_float(s)
        elif "total free energy" in s.lower():
            out["g298"] = _last_float(s)
        elif "entropy" in s.lower() and "kcal" in s.lower():
            out["s298"] = _last_float(s)
...
def _parse_scan_log(path: Path) -> list[dict[str, Any]]:
    if not path.exists():
        return []
    ...
    while idx < len(text):
        ...
        points.append({
            "point_index": p_idx,
            "coord_value": float("nan"),  # xtb scan log doesn't always emit explicit coord
            "energy": energy,
            "geometry_xyz": "\n".join(xyz_lines),
        })
        p_idx += 1
...
def _parse_fukui(stdout: str) -> tuple[list[float], list[float], list[float]]:
    fp: list[float] = []
    fm: list[float] = []
    fz: list[float] = []
    in_block = False
