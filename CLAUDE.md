...
To add a hook: create the YAML file, add an entry in `BUILTIN_REGISTRARS` (`services/agent-claw/src/core/hook-loader.ts`), implement the registrar, and write a vitest test. The loader returns a `HookLoadResult.skipped` list at boot, so a YAML without a registrar (or vice versa) surfaces immediately.
