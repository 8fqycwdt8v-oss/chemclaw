...
3. **Finish by shipping the branch end-to-end automatically.** For anything bigger than a trivial one-line tweak: open a PR with `gh pr create`, wait for CI to go green (poll via `Monitor`/`gh pr checks` — fix real failures, don't bypass), then `gh pr merge <N> --merge`, then delete the remote branch (`git push origin --delete <branch>`), the worktree (`git worktree remove`), and the local branch (`git branch -D`). The PR description is the audit trail — write a real one — but the **review-and-merge step is the agent's job, not the user's**. The user does NOT review PRs manually; leaving an open PR sitting on origin counts as unfinished work. Pre-existing flakes that block CI: skip-list / xfail them and note in `BACKLOG.md` rather than waiting on a human. Stacked-PR situations: merge each as it goes green; retarget the next via `gh pr edit --base main` after its parent merges. The `superpowers:finishing-a-development-branch` skill walks the same flow if you want a checklist.
...
5. **Log every "worth doing later" item to `BACKLOG.md`.** The flip side of rule 2 is that real follow-ups still need to land somewhere. When you discover or deliberately defer something useful — a follow-up PR you didn't ship, a related cleanup you scoped out, a future feature suggested by the work you just did, a test you noticed was missing, a TODO you saw in passing — append a one-line entry to `BACKLOG.md` at the repo root before finishing the session. Format: one bullet per item, prefixed with the area in brackets, terse, scannable. Example: `- [agent-claw/skills] expose MAX_ACTIVE_SKILLS via config_settings`. Don't write justifications, novellas, or PR-sized plans — just enough that a future agent / human can rediscover the idea. Create `BACKLOG.md` if it doesn't exist yet (no separator headers, no per-PR dating; the file is a flat append-only list). The point is that good ideas don't evaporate when you scope them out of the current task.
...
- **Phase F.1** (chemistry MCPs): chemistry services on the `chemistry` profile: askcos (8007), aizynth (8008), chemprop (8009), xtb (8010), synthegy-mech (8011), sirius (8012). The admetlab adapter has been removed from this build. **synthegy-mech** is LLM-guided mechanism elucidation via A* over arrow-pushing primitives (Bran et al., *Matter* 2026, [10.1016/j.matt.2026.102812](https://doi.org/10.1016/j.matt.2026.102812)); vendored from `github.com/schwallergroup/steer` (MIT). Ionic chemistry only — radical/pericyclic inputs surface a `warnings` entry. All LLM traffic flows through the central LiteLLM proxy. Per-request server-side wall-clock cap at 270s (30s under the agent-side `TIMEOUT_SYNTHEGY_MECH_MS = 300_000`). xTB-based intermediate energy validation is opt-in via `validate_energies=true`.
