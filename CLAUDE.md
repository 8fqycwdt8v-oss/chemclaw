...
- **C**: working-memory compactor; `contextual_chunker` projector; `skill_library` + maturity tiers; 4-signal confidence ensemble (verbalized 0.30 + cross_model 0.25 + bayesian 0.25 + calibrated chemprop std 0.20). Defined in `services/agent-claw/src/core/confidence.ts`; thresholds in `config_settings` (`confidence.threshold.{foundational,high,medium}`, defaults 0.85/0.65/0.40); per-computation contribution telemetry via `logEnsembleSignals`.
...
- **Knowledge-wiki projection layer (ADR 012, in progress)**: a human-readable, citation-traced wiki over the bi-temporal KG — the "molecule layer" complementing the KG's "atom layer". The agent reads a synthesised page instead of re-deriving knowledge per query; humans edit a page and the edit flows back as an `expert_validated` source; a future linter keeps pages honest about staleness/contradictions. It is another event-sourced, replayable derived view — zero churn on the KG/event spine. **Phase 0** (`docs/adr/012-knowledge-wiki-projection.md`, `docs/plans/knowledge-wiki-projection.md`): `knowledge_articles` / `_revisions` / `_citations` tables (`db/init/58_knowledge_wiki.sql`, RLS+FORCE, `knowledge_article_created/_revised/_archived` ingestion events). **Phase 1**: four builtins (`read_article`, `list_articles`, `upsert_article`, `request_article` — registered always, gated at call time by the `wiki.enabled` feature flag, default OFF), the `wiki-human-block-guard` pre_tool hook (denies an agent authoring `<!-- human:begin -->` blocks), and `GET/PATCH /api/articles[/:id]` (human edits set `has_human_edits`, bump revision+etag, write a revision row; 409 on etag conflict). **Phase 2a**: the `wiki_pages` projector (`services/projectors/wiki_pages/`) — a regular `ingestion_events` consumer (no new `kg_fact_written` event) that creates `dirty` stub pages for `project/` / `campaign/` / `document/` entities and marks pages dirty as backing data changes, propagating `fact_invalidated` to citing pages via the citation reverse-index; wired into compose (`profiles: ["full"]`) + helm + `make run.wiki-pages`; `db/init/59_wiki_pages_consumer.sql` registers it in `ingestion_event_catalog`. **Phase 2b**: the `wiki_regen` daemon (`services/optimizer/wiki_regen/`) — polls for `dirty` entity-backed pages (debounced + per-hour rate-capped), gathers a Postgres context per kind, calls central LiteLLM with the `wiki.synthesis` prompt (`prompt_registry`, seeded in `db/seed/07_wiki_synthesis_prompt.sql`, with a built-in fallback), parses inline `[fact:…]`/… citations into `knowledge_article_citations`, writes a `knowledge_article_revisions` row (`author_kind='projector'`), preserves `human:*` blocks, and prepends to the `log` page; knobs in `db/init/60_wiki_regen_config.sql` (read as env vars today); compose `profiles: ["full"]`, helm `projectors.wikiRegen` (default `enabled: false` — LLM-cost-incurring), `make run.wiki-regen`. **Phase 3a**: the `wiki_kg` projector (`services/projectors/wiki_kg/`) — a direct-driver projector (via `services/projectors/common/neo4j_client.py`, like `kg_documents`) that mirrors `knowledge_articles` rows as `:WikiPage` nodes in Neo4j with `:SUMMARIZES` edges to the backing entity node (for the labels the KG already nodes — `Compound`/`NCEProject`) and `:GROUNDS` edges to the `:Fact` nodes each revision cites (bi-temporal: dropped facts get `invalidated_at`; `index`/`log` pages skipped); deterministic edge ids, replay-safe; compose `profiles: ["full"]`, helm `projectors.wikiKg`, `make run.wiki-kg`. **Phase 3b**: the `wiki_search_index` projector (`services/projectors/wiki_search_index/`) + `wiki_chunks` table (`db/init/61_wiki_chunks.sql`, RLS transitive through the parent article) — on `knowledge_article_created`/`_revised` it heading-aware-chunks the article's current body, embeds via `mcp-embedder` (BGE-M3, same as `document_chunks`), and DELETE-then-INSERTs into `wiki_chunks` so it always holds exactly the current revision's chunks; `_archived` → DELETE; stub body → no-op; embedder 4xx → leave old chunks; replay-safe; compose `profiles: ["full"]` (`depends_on` mcp-embedder), helm `projectors.wikiSearchIndex`, `make run.wiki-search-index`. **Phase 3c**: `search_knowledge` gained an `include_wiki` input (default true) and a dense+sparse arm over `wiki_chunks` (joined to `knowledge_articles`, `status='current'`, RLS-scoped) — in `hybrid` mode all four arms RRF-fuse; hits now carry `kind: "document" | "wiki"` (wiki hits set `slug`/`article_id` + a `knowledge_article` citation — added to `Citation.source_kind` in `core/types.ts`); `retrieve_related` passes `include_wiki: true` so wiki pages surface there too. `query_provenance` ← `:WikiPage` is a BACKLOG follow-up. **Phase 4a**: the `wiki_linter` cron (`services/optimizer/wiki_linter/`) — every `WIKI_LINTER_POLL_HOURS` (default 6) it creates `dirty` stub `project/<internal_id>` pages for projects lacking one (`lint:missing_page`; `wiki_regen` then fills them), logs orphan `topic/` pages (no inbound `[article:…]` citation), and rebuilds the `index` page (a Karpathy-style catalog grouped by kind — written only when changed, no `revision` bump) + appends a `log` entry; pure-Postgres, no LLM; compose `profiles: ["full"]`, helm `projectors.wikiLinter`, `make run.wiki-linter`, `services/optimizer/wiki_linter/*` in the coverage `omit` + `test_wiki_linter.py` in the CI pytest list (like `wiki_regen`). **Phase 4b-i**: surfaces — `POST /api/admin/articles/:id/maturity` (`services/agent-claw/src/routes/admin/admin-articles.ts`, `guardAdmin` + `appendAudit("knowledge_article.maturity")`, bumps `etag` but **not** `revision` — maturity is curation metadata, not page content), the `/wiki <query>` slash verb (`STREAMABLE_VERBS` + `VERB_TO_SKILL.wiki = "wiki_curator"`) activating the `wiki_curator` skill (`skills/wiki-curator/SKILL.md`, scoped to `list_articles`/`read_article`/`upsert_article`/`request_article` + `search_knowledge`/`query_kg`/`retrieve_related`/`synthesize_insights`; forbidden from authoring entity-backed kinds and from writing `<!-- human:begin -->` markers), the `wiki.contradiction` prompt seed (`db/seed/08_wiki_contradiction_prompt.sql`, consumed by Phase 4b-ii), and the curation runbook (`docs/runbooks/knowledge-wiki-curation.md`). **Phase 4b-ii**: `wiki_linter` gained two Neo4j-backed sweeps (via `services/projectors/common/neo4j_client.py`, gated on `NEO4J_URI`): `_sweep_stale_citations` — for every `current` `dirty=false` page, ask Neo4j which of its cited `:Fact` nodes have `invalidated_at IS NOT NULL` or have only invalidated incoming `:CITES` edges, and re-dirty the citing page with `dirty_reason='lint:stale_citation'`; and `_sweep_contradictions` — Cypher group-by on `(subject_label, subject_id_value, predicate)` finds `:Fact` clusters with ≥ `wiki_linter_contradiction_min_objects` distinct active object values, stamps the subject+predicate+fact_ids into `entity_ref`, and creates a `contradiction/<slug>` stub with `dirty_reason='lint:contradiction'`. `wiki_regen` gained `_ctx_contradiction` (reads `claim_a`/`claim_b` straight from the linter's `entity_ref` — no extra Neo4j call), a per-kind prompt selector (`contradiction` → `wiki.contradiction`, else `wiki.synthesis`), and a built-in `_FALLBACK_CONTRADICTION_PROMPT`. Remaining (Phase 5): observability (Grafana panels); plus follow-ups (compound/reaction-family auto-stubbing, `:SUMMARIZES` for more labels, project-page `group_id`, `ConfigRegistry` wiring, page `confidence_score`, on-read regen, `query_provenance` ← `:WikiPage`) — tracked in `BACKLOG.md` and the plan doc.
- **Universal knowledge accumulation — Phase 0** (spec: `docs/plans/2026-05-14-universal-knowledge-accumulation.md`; Phase 0 plan: `docs/plans/2026-05-14-universal-knowledge-accumulation-phase-0.md`): foundational substrates for the autonomous knowledge-accumulation loop. **Phase 0 ships scaffold only — the feature flag `kg.auto_extraction.enabled` defaults OFF and the registry ships empty, so zero facts are emitted until Phase 1 lands per-source extractors.** New tables: `facts` (canonical RLS-enforced fact store, bi-temporal, with `derivation_class` capability enum OBSERVED→COMPUTED→INTERPRETED→HYPOTHESIZED→ABSTRACTED + multiplicative-decay confidence), `extraction_registry` (dispatch table for tool→extractor mapping), `investigation_queue` (RLS-scoped scored deferred work), `investigation_budget_usage` (daily LLM/CPU budget counters). New columns: `derivation_class` + `confirmed_by FK→facts` on existing fact-bearing tables (reactions/hypotheses/artifacts/compute_results). New ingestion events cataloged: `tool_invocation_complete`, `extracted_fact`, `anomaly_observed`, `pattern_detected`, `interpretation_proposed`, `investigation_requested`, `test_planned`, `external_data_fetched`. New seeds in `db/seed/09_universal_extraction_config.sql`: feature flag (default off) + 14 `config_settings` knobs (extractor reliability factors per class, investigation scoring weights, budgets, sweep intervals, max derivation depth) + 4 `prompt_registry` placeholders for Phase 3+. New TypeScript hook `services/agent-claw/src/core/hooks/tool-invocation-emitter.ts` (post_tool + post_tool_failure, wired into BUILTIN_REGISTRARS; gated by feature flag; MIN_EXPECTED_HOOKS bumped 27→28). New Python projector `services/projectors/tool_result_extractor/` (registry-driven dispatch to per-source extractor modules; no-op when registry empty; compose+helm+Makefile wired). Two new agent builtins (`promote_to_kg` for INTERPRETED/HYPOTHESIZED/ABSTRACTED facts with per-class confidence caps; `request_investigation` for manual queue enqueues; both bump MIN_EXPECTED_BUILTINS 94→96). RLS write policies in `db/init/68_facts_app_write_policies.sql` let `chemclaw_app` insert agent-promoted facts and manual investigation requests while denying forged OBSERVED/COMPUTED classes. Integration smokes (`tests/integration/test_phase_0_*.py`) verify the default-off no-extractor path is a clean no-op and the promote+request happy path lands under RLS. Phases 1–7 (chemistry MCP extractors, document/ELN/LOGS extraction, investigation scorer, anomaly/pattern detectors, hypothesis former, test planner, agent-internal conclusion extraction, wiki anomaly/pattern sections) get their own plans as Phase 0 lands.
...
```
services/agent-claw  npm test           → 1497 passed | 12 skipped (195 files)
