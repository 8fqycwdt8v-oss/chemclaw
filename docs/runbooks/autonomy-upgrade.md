...
1. ~~**MCP Bearer-token end-to-end wire**~~ — SHIPPED. `signMcpToken` is threaded through `postJson` / `getJson` via AsyncLocalStorage; every `/tools/*` route now goes through the `require_mcp_token` middleware in `services/mcp_tools/common/app.py`. `MCP_AUTH_REQUIRED=true` is the production default; `MCP_AUTH_DEV_MODE=true` is the dev-only escape (per CLAUDE.md "Secrets and egress").
2. ~~**Reanimator → agent JWT**~~ — SHIPPED. The reanimator mints its own JWT (`agent:resume` scope) and POSTs to `/api/internal/sessions/:id/resume`, which trusts only the signed `claims.user`. No `x-user-entra-id` forgery surface.
